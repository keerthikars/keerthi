<h1 align="center">Hi 👋 I'm KEERTHIKA R</h1>

<h3 align="center">🚀 Electronics and Communication Engineering | Full Stack Developer | Problem Solver</h3>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=26&duration=3000&color=00F7FF&center=true&vCenter=true&width=700&lines=ECE+Student;Full+Stack+Developer;DSA+Problem+Solver;Building+AI-Powered+Applications">
</p>

---

<p align="center">
<img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="400">
</p>

---

# 👨‍💻 About Me

- 🎓 B.E in **Electronics and Communication**
- 🏫 **Sri Eshwar College of Engineering** (2024 – 2028)
- 💻 Passionate about **Full Stack Development**
- 📈 Solved **200+ LeetCode** & **1300+ Skillrack** problems
- 🌱 Currently exploring **REST APIs, React and Modern Web Technologies**
- ⚡ Love building real-world applications that combine AI with Web Development

---

# 🚀 Technical Skills

## 💻 Programming Languages

![C](https://img.shields.io/badge/C-blue?style=for-the-badge)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus)
![Java](https://img.shields.io/badge/Java-red?style=for-the-badge&logo=openjdk)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript)
![Python](https://img.shields.io/badge/Python-yellow?style=for-the-badge&logo=python)
![SQL](https://img.shields.io/badge/SQL-blue?style=for-the-badge&logo=mysql)

---

## 🌐 Frontend

![HTML](https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)

---

## ⚙️ Backend

![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js)
![Express](https://img.shields.io/badge/Express-black?style=for-the-badge&logo=express)
![REST API](https://img.shields.io/badge/REST-API-blue?style=for-the-badge)

---

## 🤖 Machine Learning

![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn)
![XGBoost](https://img.shields.io/badge/XGBoost-grey?style=for-the-badge)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas)

---

## 🗄 Database

![MySQL](https://img.shields.io/badge/MySQL-blue?style=for-the-badge&logo=mysql)
![MongoDB](https://img.shields.io/badge/MongoDB-green?style=for-the-badge&logo=mongodb)

---

## 🛠 Tools

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode)

---

# 💼 Internship Experience

## 👨‍💼 Employee Attrition Prediction & HR Analytics System

**Role:** Full Stack Developer & Machine Learning Intern

### Highlights

- Built an AI-powered employee attrition prediction system
- Developed HR analytics dashboards for data visualization
- Generated employee retention insights using Machine Learning
- Implemented secure authentication and responsive UI

**Tech Stack**

React • Node.js • Express • MySQL • Python • Scikit-learn • XGBoost

---

# 📂 Featured Projects

## 📦 Campus Store Inventory Management System

- Inventory and stock management platform
- Product management and billing system
- Secure authentication
- Responsive web interface

**Tech Stack:** React, Node.js, Express, MongoDB

---

## 🎯 Course Outcome & Skill Mapping System

- AI-powered student skill analysis platform
- Personalized learning path recommendation
- REST API based architecture
- Performance analytics dashboard

**Tech Stack:** React, Node.js, Express, MongoDB, Python

---

## 🌊 FloatChat – AI Powered ARGO Ocean Data Analysis System

- Natural language querying of ARGO ocean data
- AI-powered chatbot interface
- Interactive charts and visualizations
- Ocean temperature, salinity and pressure analysis

**Tech Stack:** Python, Streamlit, Flask, ARGO Dataset, Pandas

---

# 🏆 Achievements

🥈 First Runner-Up – Crack It Competition (Sri Eshwar College of Engineering)

💡 Ideathon Participant – Sri Eshwar College of Engineering

🎨 Freshathon Participant – Sri Eshwar College of Engineering

---

# 📜 Certifications

- Introduction to C
- Introduction to Data Structure and Algorithm
- SQL for Beginners
- Internet of Things (IoT)
- Bootcamp on Artificial Intelligence

---

# 💻 Coding Profiles

- 🔥 LeetCode – **200+ Problems Solved**
- 💪 Skillrack – **1300+ Problems Solved**

---


# 🧠 LeetCode Stats

<p align="center">
<img src="https://leetcard.jacoblin.cool/Keerthika_robi?theme=dark&font=baloo&ext=heatmap"/>
</p>

---


<h3 align="center">
⭐ Thanks for visiting my profile! ⭐
</h3>
